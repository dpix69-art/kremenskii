...
export default function SoundProjectDetailPage() {
  ...
  // --- ФОТО: поддерживаем и string[], и {url,alt?}[] ---
  const photos =
    Array.isArray(s.photos)
      ? s.photos
          .map((p: any) => (typeof p === "string" ? { url: p } : p))
          .filter((p: any) => typeof p?.url === "string" && p.url.trim().length > 0)
      : [];

  // --- ТРЕКИ: поддерживаем и string[], и {title,duration?}[] (скрыты по умолчанию) ---
  const tracks =
